...
function handleWs(msg) {
  switch (msg.type) {
    ...
    case 'room:leave':
      if (S.currentRoom?.id === msg.roomId) {
        S.currentRoom.members = S.currentRoom.members.filter(m => m.kzenId !== msg.user?.kzenId && m.username !== msg.username);
        updateFocusMembers();
      }
      break;
    case 'room:phase':
  ...
  el.innerHTML = r.rooms.map(room => `
    <div class="room-card" data-id="${room.id}">
      <div>
        ...
        <div class="room-meta">${escapeHtml(room.subject)} · voice ${escapeHtml(room.voiceCycle)}${room.cameraRequired ? ' · cam req' : ''}<span class="room-phase ${room.voicePhase}">${room.voicePhase === 'break' ? 'voice open' : 'focus'}</span></div>
      </div>
      <div class="room-count"><em>${room.members}</em> / ${room.maxPeople}</div>
    </div>
  `).join('');
  el.querySelectorAll('.room-card').forEach(card => {
    card.addEventListener('click', () => {
      if (S.isGuest) return toast('guest mode: sign up to join rooms');
      startFocus(card.dataset.id);
    });
  });
}
...
function startFocus(roomId, opts = {}) {
  ...
  wsSend({ type: 'session:start', subject: opts.subject, soloCommit: opts.soloCommit || 0 });
  goTo('focus');
...
function focusTick() {
  ...
  const inRoomFocus = S.focus.roomId && S.roomPhase === 'focus';
...
async function toggleCamera() {
  ...
  const ok = confirm('camera stays on your device. no frames, video, or images ever leave your browser. use it?');
  if (!ok) return;
  try {
...
function stopFocus(success) {
  ...
  if (committed && !completed) {
    ...
    toast(`session: ${Math.floor(S.focus.elapsed / 60)} min · +${xpGained} xp`);
  }
