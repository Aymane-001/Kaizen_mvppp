...
const S = {
  ...
  focus: {
    ...
    roomXpEarned: 0, phoneDetections: 0, distractionEvents: 0, burstTimer: null,
  },
...
function goTo(name) {
  ...
  if (name === 'home') { refreshHome(); loadDailyTasks(); }
...
function applyTheme(theme) {
  ...
  document.documentElement.classList.toggle('gamer-mode', isGamer);
  if (theme === 'ink' || isGamer) document.documentElement.removeAttribute('data-theme');
  else document.documentElement.setAttribute('data-theme', theme);
  $$('.theme-swatch').forEach(sw => sw.classList.toggle('active', sw.dataset.theme === theme));
  if (S.user) S.user.theme = theme;
  updateGamerLabels();
}

function updateGamerLabels() {
  const isGamer = document.documentElement.classList.contains('gamer-mode');
  const battleLabel = $('#battle-mode-label');
  if (battleLabel) battleLabel.textContent = isGamer ? '⚔ BATTLE' : 'duel';
  const pathBattle = document.querySelector('.path-card[data-action="battle"] .path-title');
  if (pathBattle) pathBattle.textContent = isGamer ? 'BATTLE' : 'duel';
  const pathBattleSub = document.querySelector('.path-card[data-action="battle"] .path-sub');
  if (pathBattleSub) pathBattleSub.textContent = isGamer ? 'DOMINATE a rival. winner takes ALL.' : 'race a friend, winner takes xp';
}
...
function handleWs(msg) {
  switch (msg.type) {
    ...
    case 'friend:status':
      ...
      break;
    case 'friend:online':
      if (S.focus.active) _showSocialTick(msg.kzenId + ' just came online \u00B7 someone else is at it');
      if (S.screen === 'friends') loadFriends();
      break;
    case 'battle:invite':
      S.pendingBattle = msg;
...
async function refreshHome() {
  ...
  const _lbl = !S.user ? null : S.user.streak >= 100 ? 'machine' : S.user.streak >= 30 ? 'grinder' : S.user.streak >= 7 ? 'regular' : null;
  const _lblEl = $('#identity-label');
  if (_lblEl) { _lblEl.textContent = _lbl ? 'kaizen ' + _lbl : ''; _lblEl.style.display = _lbl ? 'inline' : 'none'; }
  if (S.user && S.user.streak > 0 && (S.user.xp_daily === 0) && !S.isGuest) {
    const msLeft = new Date(new Date().toDateString()).getTime() + 86400000 - Date.now();
    const hoursLeft = Math.floor(msLeft / 3600000);
    if (hoursLeft < 8) setTimeout(() => toast('\uD83D\uDD25 ' + S.user.streak + '-day streak at risk \u00B7 ' + hoursLeft + 'h left today', 6000), 1800);
  }
...
function setupSettings() {
  ...
  $('#show-pw-form').addEventListener('click', () => {
    $('#pw-form').style.display = 'block';
    $('#show-pw-form').style.display = 'none';
  });
  ...
  $('#pw-form').addEventListener('submit', async (e) => {
    ...
    e.target.reset();
    $('#pw-form').style.display = 'none';
    $('#show-pw-form').style.display = 'block';
  });
...
function refreshSettings() {
  ...
  $('#goal-range').value = S.user.daily_goal_xp || 1800;
  $('#goal-value').textContent = (S.user.daily_goal_xp || 1800) + ' xp';
  updateGamerLabels();
...
function startFocus(roomId, opts = {}) {
  ...
  S.focus.xp = 0;
  S.focus.roomXpEarned = 0;
  S.focus.phoneDetections = 0;
  S.focus.distractionEvents = 0;
  S.focus.mult = 1;
  S.focus.cam = false;
...
let _ambCtx = null, _ambGain = null, _ambNodes = [], _ambActive = null;

function _stopAmb() {
  if (_ambGain) { try { _ambGain.gain.setTargetAtTime(0, _ambCtx.currentTime, 0.4); } catch { } }
  setTimeout(() => { try { _ambCtx?.close(); } catch { } _ambCtx = null; _ambGain = null; _ambNodes = []; }, 700);
  _ambActive = null;
  $$('.amb-btn').forEach(b => b.classList.remove('active'));
}

function toggleAmbient(type) {
  if (_ambActive === type) { _stopAmb(); return; }
  _stopAmb();
  setTimeout(() => _startAmb(type), 300);
}

function stopAmbient() { _stopAmb(); }

function _startAmb(type) {
  try {
    ...
    _ambCtx = ctx; _ambGain = master; _ambActive = type;

    if (type === 'rain') {
      // layered rain: high filtered white noise + low rumble
      const hiBuf = _whiteBuf(ctx, 4);
      const loBuf = _whiteBuf(ctx, 4);
      const hiSrc = ctx.createBufferSource(); hiSrc.buffer = hiBuf; hiSrc.loop = true;
      const loSrc = ctx.createBufferSource(); loSrc.buffer = loBuf; loSrc.loop = true;
      const hiF = ctx.createBiquadFilter(); hiF.type = 'bandpass'; hiF.frequency.value = 3500; hiF.Q.value = 0.4;
      const loF = ctx.createBiquadFilter(); loF.type = 'lowpass'; loF.frequency.value = 200;
      const hiG = ctx.createGain(); hiG.gain.value = 0.55;
      const loG = ctx.createGain(); loG.gain.value = 0.25;
      hiSrc.connect(hiF); hiF.connect(hiG); hiG.connect(master);
      loSrc.connect(loF); loF.connect(loG); loG.connect(master);
      hiSrc.start(); loSrc.start();
      // occasional drip-like amplitude modulation
      const drip = ctx.createOscillator(); drip.frequency.value = 0.7;
      const dripG = ctx.createGain(); dripG.gain.value = 0.08;
      drip.connect(dripG); dripG.connect(master); drip.start();
      _ambNodes = [hiSrc, loSrc, drip];
    } else if (type === 'cafe') {
      // busy café: layered brown noise + mid-range chatter simulation
      const buf1 = _brownBuf(ctx, 5);
      const buf2 = _brownBuf(ctx, 5);
      const s1 = ctx.createBufferSource(); s1.buffer = buf1; s1.loop = true;
      const s2 = ctx.createBufferSource(); s2.buffer = buf2; s2.loop = true; s2.loopStart = 1.3;
      const f1 = ctx.createBiquadFilter(); f1.type = 'peaking'; f1.frequency.value = 1200; f1.gain.value = 8; f1.Q.value = 0.5;
      ...
      // murmur LFO for bustle feel
      const lfo = ctx.createOscillator(); lfo.frequency.value = 1.8;
      const lfoG = ctx.createGain(); lfoG.gain.value = 0.12;
      lfo.connect(lfoG); lfoG.connect(master); lfo.start();
      s1.start(); s2.start();
      _ambNodes = [s1, s2, lfo];
    } else if (type === 'library') {
      // library: very low, almost-silent pink noise + occasional paper rustle sim
      const buf = _pinkBuf(ctx, 6);
      ...
      // white noise (pure white noise — minimal/clean)
      const buf = _whiteBuf(ctx, 3);
      const src = ctx.createBufferSource(); src.buffer = buf; src.loop = true;
  ...
  } catch (e) { toast('sound unavailable'); }
}

function _whiteBuf(ctx, secs) {
...
function _brownBuf(ctx, secs) {
  const len = ctx.sampleRate * secs;
  const buf = ctx.createBuffer(1, len, ctx.sampleRate);
  const d = buf.getChannelData(0); let v = 0;
  for (let i = 0; i < len; i++) { v += (Math.random() * 2 - 1) * 0.12; v = Math.max(-1, Math.min(1, v * 0.998)); d[i] = v; }
  return buf;
}

function _pinkBuf(ctx, secs) {
  const len = ctx.sampleRate * secs;
  const buf = ctx.createBuffer(1, len, ctx.sampleRate);
  const d = buf.getChannelData(0);
...
const _detect = { blaze: null, coco: null, loading: false, lastAlert: 0 };

const SARCASTIC = [
  "Your phone won't take the exam. Put it down.",
  "Fascinating. Still not studying though.",
  "Whatever's over there — it's not your grade.",
  "Bold strategy. Let's see how that works out.",
  "Your future self just cringed. Get back to work.",
  "The ceiling has seen this before. It's not impressed.",
  "Phone down. Brain on. Now.",
  "Not studying is also a choice. A terrible one.",
  "Come on. You literally opened a study app.",
  "Eyes on the work. The phone will survive without you.",
];

function _saySarcastic(reason) {
  const now = Date.now();
  if (now - _detect.lastAlert < 45000) return;
  _detect.lastAlert = now;
  const line = SARCASTIC[Math.floor(Math.random() * SARCASTIC.length)];
  const prefix = reason === 'phone' ? '📱 ' : reason === 'away' ? '👀 ' : '😑 ';
  toast(prefix + line, 5000);
...
async function _loadDetectModels() {
  if (_detect.loading || (_detect.blaze && _detect.coco)) return;
  _detect.loading = true;
  console.log('kaizen: loading AI models...');
  try {
    // ensure tfjs is ready and using webgl
    if (typeof tf !== 'undefined') {
      await tf.ready();
      await tf.setBackend('webgl').catch(() => console.log('kaizen: webgl backend not available, using default'));
    }

    const [blaze, coco] = await Promise.all([
      typeof blazeface !== 'undefined' ? blazeface.load() : (console.warn('blazeface not found in window'), null),
      typeof cocoSsd !== 'undefined' ? cocoSsd.load() : (console.warn('cocoSsd not found in window'), null),
    ]);

    _detect.blaze = blaze;
    _detect.coco = coco;

    if (_detect.blaze) console.log('kaizen: BlazeFace loaded');
    if (_detect.coco) console.log('kaizen: COCO-SSD loaded');

  } catch (e) {
    console.warn('kaizen: ML models failed to load', e);
  }
  _detect.loading = false;
}

async function _runSmartDetection() {
  const video = $('#cam-video');
  if (!S.focus.cam || !video || video.videoWidth === 0) return;

  // fallback to basic pixel check if models not ready
  if (!_detect.blaze) { 
    console.log('kaizen: blaze not loaded, using basic detection');
    _runBasicDetection(video); 
    return; 
  }
  ...
  try {
    const [faces, objects] = await Promise.all([
      _detect.blaze.estimateFaces(video, false),
      _detect.coco ? _detect.coco.detect(video) : Promise.resolve([]),
    ]);

    // 1. Phone in frame?
    const phone = objects.find(o => o.class === 'cell phone' && o.score > 0.5);
    if (phone) { 
      console.log('kaizen: phone detected', phone.score);
      distracted = true; 
      reason = 'phone'; 
    }

    // 2. No face → walked away
    else if (faces.length === 0) { 
      console.log('kaizen: no faces detected');
      distracted = true; 
      reason = 'away'; 
    }

    else {
      // 3. Head pose via BlazeFace landmarks
      const lm = faces[0].landmarks; 
      const rEye = lm[0], lEye = lm[1], nose = lm[2];

      const lDist = Math.abs(lEye[0] - nose[0]);
      const rDist = Math.abs(rEye[0] - nose[0]);
      const total = lDist + rDist;
      const yawAsym = total > 0 ? Math.abs(lDist - rDist) / total : 0;

      const bbox = faces[0];
      const faceH = (bbox.bottomRight[1] - bbox.topLeft[1]) || 1;
      const eyeMidY = (rEye[1] + lEye[1]) / 2;
      const pitchRatio = (nose[1] - eyeMidY) / faceH;

      if (yawAsym > 0.42) { 
        console.log('kaizen: head turned (yawAsym)', yawAsym);
        distracted = true; 
        reason = 'away'; 
      }
      else if (pitchRatio < 0.10) { 
        console.log('kaizen: looking away/up (pitchRatio)', pitchRatio);
        distracted = true; 
        reason = 'away'; 
      }
    }
  } catch (e) {
    console.warn('kaizen: detection error', e);
    _runBasicDetection(video);
    return;
  }

  _applyDistraction(distracted, reason);
}

// pixel-diff fallback (used when models not loaded yet)
let _basicLastFrame = null, _basicStillCount = 0;
function _runBasicDetection(video) {
  const canvas = document.createElement('canvas');
  canvas.width = 60; canvas.height = 45;
  const ctx = canvas.getContext('2d');
  ctx.drawImage(video, 0, 0, 60, 45);
  const frame = ctx.getImageData(0, 0, 60, 45).data;
  ...
  if (_basicLastFrame) {
    let diff = 0;
    for (let i = 0; i < frame.length; i += 16) diff += Math.abs(frame[i] - _basicLastFrame[i]);
    tooStill = diff / (frame.length / 16) < 1.5;
  }
  _basicLastFrame = frame;
  if (brightness < 30 || tooStill) _basicStillCount++; else _basicStillCount = 0;
  _applyDistraction(_basicStillCount > 4, 'away');
}

function _applyDistraction(distracted, reason) {
  if (distracted !== S.focus.distracted) {
    S.focus.distracted = distracted;
    if (distracted) {
      S.focus.distractionEvents = (S.focus.distractionEvents || 0) + 1;
      if (reason === 'phone') S.focus.phoneDetections = (S.focus.phoneDetections || 0) + 1;
    }
    const cs = $('#cam-status');
    if (cs) {
      cs.textContent = distracted
        ? (reason === 'phone' ? 'put the phone down' : reason === 'away' ? 'where did you go?' : 'distracted')
        : 'focused';
      cs.classList.toggle('distracted', distracted);
    }
  }
  if (distracted) _saySarcastic(reason);
}

function startBasicFocusDetection() {
  _basicLastFrame = null;
  _basicStillCount = 0;
...
function stopFocus(success) {
  ...
  clearTimeout(S.focus.burstTimer);
  stopCamera();
  stopAmbient();
  hideMethodPrompt();
  $('#cam-pip').style.display = 'none';
  ...
  const _dur = Math.floor(S.focus.elapsed / 60);
  const _camSec = S.focus.camSeconds;
  const _camDist = S.focus.camDistractedSec;
  const _phones = S.focus.phoneDetections || 0;
  const _distracts = S.focus.distractionEvents || 0;
  S.focus.roomId = null;
...
function showSessionEnd(dur, xp, broken, camSec, camDistSec, phones, distracts) {
  goTo('home');
  $('#se-duration').textContent = dur + ' min';
  const xpRow = $('#se-xp-row');
  if (xpRow) xpRow.innerHTML = broken
    ? '<em style="color:var(--danger)">commit broken \u00B7 0 xp</em>'
    : '+<em id="se-xp">' + xp + '</em> xp earned';
  const wrap = $('#se-score-wrap');
  if (camSec > 30) {
    const score = Math.max(0, Math.min(100, Math.round(((camSec - camDistSec) / Math.max(1, camSec)) * 100) - phones * 8));
    const tiers = [[90, 'locked in \uD83D\uDD12', '#4ade80'], [75, 'solid session', '#E8C87A'], [55, 'decent, room to grow', '#facc15'], [30, 'a bit scattered', '#fb923c'], [0, 'rough one', '#f87171']];
    const [, tag, color] = tiers.find(([min]) => score >= min);
    const pct = Math.round(((camSec - camDistSec) / Math.max(1, camSec)) * 100);
    if ($('#se-score-tag')) $('#se-score-tag').textContent = tag;
    if ($('#se-score-detail')) $('#se-score-detail').textContent = pct + '% face present \u00B7 ' + phones + ' phone alert' + (phones !== 1 ? 's' : '') + ' \u00B7 ' + distracts + ' distraction' + (distracts !== 1 ? 's' : '');
    if (wrap) wrap.style.display = 'block';
    setTimeout(() => {
      const numEl = $('#se-score'), ring = $('#se-ring');
      if (!numEl || !ring) return;
      const CIRC = 351.86;
      ring.style.stroke = color; numEl.textContent = '0'; ring.style.strokeDashoffset = CIRC;
      const t0 = performance.now();
      (function step(now) {
        const t = Math.min(1, (now - t0) / 1300);
        const e = 1 - Math.pow(1 - t, 3);
        numEl.textContent = Math.round(score * e);
        ring.style.strokeDashoffset = CIRC * (1 - (score / 100) * e);
...
function scheduleFocusBurst() {
  const delay = (7 + Math.random() * 6) * 60 * 1000;
  S.focus.burstTimer = setTimeout(() => {
    if (!S.focus.active) return;
    const bonus = 10 + Math.floor(Math.random() * 10);
    S.focus.xp += bonus;
    wsSend({ type: 'xp:tick', amount: bonus });
    toast('\u26A1 focus burst \u00B7 +' + bonus + ' xp', 3000);
    updateFocusDisplay();
    scheduleFocusBurst();
  }, delay);
}

function _showSocialTick(msg) {
  const el = $('#social-ticker');
  if (!el) return;
  el.textContent = msg;
...
function setupModals() {
  ...
  document.getElementById('se-done')?.addEventListener('click', () => hideModal('modal-session-end'));
}
